...
- Backend fixture startup can be blocked by the benchmark's own in-page SSE fetch if it races the app's selected-live EventSource on the exact same URL. Add a cache-busting probe query and wait briefly for SSE response headers before posting the fixture so startup failures are reported as transport problems rather than fixture POST aborts.
- Backend `batch` profile is now a controlled negative benchmark shape: after startup stabilization it preserves text/reasoning events but intentionally collapses DOM to 3 positive updates and SSE text-events-per-chunk p90 to 4, proving the SSE/DOM gates catch batching.
- `pibo debug web scenario streaming-benchmark --expect-regression <substring>` turns controlled negative profiles into required assertions: expected regression substrings must match, while unexpected or missing expected regressions still fail with `--assert`. For the backend `batch` profile, include DOM update, DOM max jump, and SSE text-events-per-chunk substrings because SSE batching may appear depending on chunk timing.
- `pibo debug web scenario streaming-benchmark --negative-profile batch` is a shorthand for the backend batch reasoning/text controlled-negative fixture; it selects the known expected DOM/SSE batching regressions and implies assertion mode so agents do not have to copy fragile `--expect-regression` substrings by hand.
- Streaming benchmark fixture summaries now preserve fixture `scheduleMs` and schedule-gap stats. Compare fixture schedule p90 against SSE/DOM p90 to separate intentional input cadence (steady/jitter/burst/batch) from downstream transport or rendering delay.
- Streaming benchmark artifacts now include cadence lag fields that normalize DOM and SSE p90 text gaps against the deterministic fixture schedule p90. Use `domLagOverScheduleP90Ms` and `sseTextLagOverScheduleP90Ms` to tell downstream delay from intentionally jittery/bursty input cadence.
- `pibo debug web scenario streaming-benchmark --backend-fixture --compare-url <chat-url>` now runs the same deterministic backend fixture at the current Chat URL and a second Chat URL, preserving fixture-normalized DOM/SSE cadence lag for direct-vs-hosted transport comparisons. Passing a base `/apps/chat` URL reuses the current room/session path and adds `debugStreaming=1`.
- URL comparison benchmarks now gate relative hosted/direct degradation in addition to each URL's per-run health gates. `--compare-url --assert` fails if the compare URL loses SSE/selected-live events, drops smoothness by more than 15 points, or adds excessive DOM/SSE cadence lag over the deterministic fixture schedule.
- `pibo debug web scenario streaming-benchmark --backend-fixture --compare-hosted` now resolves the hosted Chat URL from `PIBO_DEV_PUBLIC_URL`, `PIBO_DEV_BASE_URL`, or repo-local `.env.developer-host`; use it for direct-vs-hosted comparisons without hard-coding hostnames.
- Use `pibo debug web scenario streaming-benchmark --backend-fixture --compare-hosted-if-configured` in portable Ralph/CI loops: it performs the hosted comparison when `PIBO_DEV_PUBLIC_URL`/`PIBO_DEV_BASE_URL` is configured, but otherwise keeps the primary benchmark artifact and records a skip warning instead of failing before CDP discovery.
- `pibo debug web scenario streaming-benchmark --provider-request-id <pr_...>` now attaches provider/Pi telemetry from `pibo debug telemetry` to the benchmark artifact: text/reasoning delta counts, delta byte/gap stats, parse/unknown counts, and first-byte/first-text latency. Use it for real-provider smoke artifacts; deterministic fixture runs can still validate the option with an unavailable provider record when the worker has no telemetry.
- Provider telemetry artifacts now derive provider-to-transport preservation ratios (`sseTextToProviderRatio`, `selectedLiveTextToProviderRatio`, reasoning equivalents, and DOM-positive/provider text ratio). Use them on real-provider smoke runs to prove downstream preservation against provider delta counts instead of comparing absolute stream counts by hand.
- Streaming benchmark `--assert` now applies provider/Pi telemetry health gates when `--provider-request-id` resolves an available provider request: parse errors, unknown events, truncated provider pages, and provider-to-SSE/selected-live text/reasoning preservation ratios below 0.95 become regressions. Missing provider telemetry remains non-fatal so deterministic worker fixtures can still validate the option without credentials.
- Streaming benchmark provider telemetry can now be attached by `--provider-session-id <ps_...>` or `--provider-turn-id <turn-or-event-id>`; the CLI resolves the latest provider request from debug telemetry metadata, so real-provider smoke runs no longer require copying `pr_...` ids by hand.
- `pibo debug web scenario streaming-benchmark --provider-selected-session` discovers provider telemetry from the currently selected Chat Web session in the attached browser, avoiding manual `ps_...` or `pr_...` copying for real-provider smoke observations. If the worker has no provider telemetry for that session, it records `provider.available=false` and remains non-fatal.
- Provider telemetry discovered by session, turn, or selected browser session should be collected after the benchmark observation window. Real-provider requests may be created or completed during the run; pre-window discovery can miss the provider baseline and leave preservation ratios unavailable.
- Multi-run streaming benchmark summaries now include live pipeline debug medians from `window.__piboStreamingDebug`: enqueue, flush, flushed event, overlay update/event, current output, trace base, and trace refresh duration/count fields. Use these alongside DOM/SSE medians to separate live-overlay batching from transport batching.
- Streaming benchmark artifacts now include live pipeline preservation ratios normalized by fixture/provider/debug expected input events. Use enqueue/flushed/overlayEvents ratios and flush/enqueue or overlayUpdates/flushed ratios to separate transport loss from live-overlay batching.
- Live pipeline ratio gates now distinguish transport preservation from React/live-overlay batching. Healthy steady reasoning/text fixtures keep flush/enqueue and overlayUpdates/flushed near `0.909`; the controlled backend `batch` profile preserves text/reasoning but drops both ratios to about `0.591`, so it should require expected live-pipeline batching regressions.
- `pibo debug web scenario streaming-benchmark --negative-profile overlay-drop` is a controlled live-pipeline preservation failure: browser EventSource and the independent SSE fetch probe still receive all text/reasoning fixture events, while a benchmark-only in-page hook drops text/reasoning before live-overlay enqueue. Use it to prove preservation-ratio gates fail separately from transport/SSE batching gates.
- Negative-profile streaming artifacts now carry `negativeProfile`, and text output prints a compact `expected regressions:` row with matched/expected/unexpected/missing counts. Use it to confirm controlled negative probes are intentional without parsing raw regression lists.
- URL comparison artifacts now preserve a shared `negativeProfile` when both primary and compare runs use the same controlled negative preset, and text output prints it. Use this when comparing controlled negative behavior across direct/hosted URLs so expected failures remain clearly intentional.
- URL comparison streaming benchmark text now prints selected-live EventSource medians and live-pipeline ratio medians for primary and compare URLs, plus deltas. Use these rows to inspect direct-vs-hosted preservation without opening the JSON artifact.
- Streaming benchmark artifacts now carry first-latency fields across layers: selected-live EventSource first text, independent SSE fetch first text/chunk, live debug first text/enqueue/flush/overlay update, and DOM first visible. Use these to separate startup delay from steady-state cadence regressions.
- URL comparison streaming benchmark text and artifacts now include cross-layer first-latency deltas for selected-live EventSource, independent SSE fetch, live debug text/enqueue/flush/overlay, DOM first visible, and provider first text when available. Use these rows to separate hosted/direct startup delay from steady cadence lag.
