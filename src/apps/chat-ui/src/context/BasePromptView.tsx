...
export function BasePromptView() {
	...
	const load = useCallback(async () => {
		setLoading(true);
		try {
			const next = await getBasePrompt();
			setSnapshot(next);
	...
	const activateMode = useCallback(async (mode: BasePromptMode) => {
		setBusy(true);
		try {
			const next = await setBasePromptMode(mode);
			setSnapshot(next);
			setViewMode(mode);
			setCustomMarkdown(next.custom.exists ? next.custom.markdown : next.library.markdown);
			setSaveState("saved");
	...
	const saveCustom = useCallback(async () => {
		...
		setSaveState("saving");
		try {
			const next = await saveCustomBasePrompt(customMarkdown);
			setSnapshot(next);
			setViewMode("custom");
		...
		<div className="flex h-full min-h-0 flex-col bg-[#101d22]">
			...
			<div className="grid min-h-0 flex-1 grid-cols-[250px_minmax(0,1fr)] max-[980px]:grid-cols-1">
				...
				<main className="flex min-h-0 flex-col gap-3 p-4">
					...
					{loading ? (
						<div className="border border-slate-800 bg-[#151f24] p-4 text-sm text-slate-400">Loading base prompt...</div>
					) : (
						<textarea
