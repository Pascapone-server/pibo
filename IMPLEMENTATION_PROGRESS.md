...
- 2026-05-16: Started PRD 05 initial interactive CLI batch (US-001..US-002). Plan: register `pibo tui:sessions`, add command help/discovery and testable startup seam, implement an Ink app shell over fake/local `CliSessionSource` with status bar, bounded transcript viewport, input line, basic keyboard capture/cancel cleanup, document manual TTY smoke steps, then run Docker typecheck and focused/full tests before marking stories passed.
- 2026-05-16: PRD 05 US-001..US-002 first Docker typecheck failed because Ink 7 key metadata exposes `ctrl` plus input text instead of `ctrlC`; updated Ctrl+C handling to check `key.ctrl && input === "c"` before rerunning validation.
- 2026-05-16: Implemented PRD 05 US-001..US-002 initial interactive CLI batch: registered `pibo tui:sessions` in `src/cli.ts`, added root discovery and command help, added `runCliSessionsUi` startup seam with `--demo`, `--session`, `--owner-scope`, and `--max-rows`, implemented `InkSessionApp`/`InkSessionAppView` over `CliSessionSource` with status bar, command hint, bounded transcript viewport, input line, Enter/Esc/backspace/Ctrl+C handling, subscription cleanup, and manual TTY smoke steps in the PRD notes.
- 2026-05-16: Added `test/cli-ui-session-app.test.mjs` covering app shell render output, keyboard reducer behavior, command help/root discovery preserving `pibo tui` and `pibo tui:routed`, and a non-TTY startup smoke seam. Validation passed in Docker: `npm run typecheck`; `npm run build && node --test test/cli-ui-session-app.test.mjs`; full `npm test` passed (421 tests). Marked `prd_05_interactive_cli_commands.json` US-001 and US-002 `passes: true`; checked off command integration tasks T4.1..T4.5.
- 2026-05-16: Reloaded Docker worker gateway after PRD 05 US-001..US-002 using container-only build/start commands; health check passed at `http://127.0.0.1:4812/apps/chat`.
- 2026-05-16: After aligning root discovery text, reran Docker validation: `npm run typecheck && npm run build && node --test test/cli-ui-session-app.test.mjs`; it passed.
- 2026-05-16: Reloaded Docker worker gateway after final PRD 05 US-001..US-002 discovery text adjustment; health check passed at `http://127.0.0.1:4812/apps/chat`.
- 2026-05-16: Created PRD 05 US-001..US-002 batch commit `feat: add PRD05 CLI sessions command shell`; see current commit for the final hash. Next dependency-order target is PRD 05 US-003 Slash Command parser and local commands.
- 2026-05-16: Started PRD 05 command-flow batch (US-003..US-006). Plan: add testable Slash Command parser/controller helpers, wire `/help`, `/status`, `/clear`, `/exit`, `/quit`, session/new/agent picker flows, normal message sending, live row refresh, and tests over fake source before marking validated stories passed.
- 2026-05-16: Implemented PRD 05 US-003..US-006 command-flow batch: added a testable Slash Command parser/controller seam, `/help`, `/status` with redaction, `/clear`, `/exit`, `/quit`, `/session` and `/agent` keyboard picker modes, `/new` create/open flow, normal message sending through `CliSessionSource`, and live compact-row refresh via open-session subscriptions.
- 2026-05-16: Updated PRD 05 manual TTY smoke notes to cover help/status, session and agent pickers, new session, normal send/live refresh, clear, quit, and Ctrl+C recovery checks.
- 2026-05-16: Validation for PRD 05 US-003..US-006: Docker `npm run typecheck` passed; `npm run build` passed; focused `node --test test/cli-ui-session-app.test.mjs` passed; full `npm test` passed (423 tests). Marked `prd_05_interactive_cli_commands.json` US-003..US-006 `passes: true`. Next dependency-order target is PRD 06 SSH/recovery hardening US-001.
- 2026-05-16: Reloaded Docker worker gateway after PRD 05 US-003..US-006. First health probe reset during startup; follow-up health check passed at `http://127.0.0.1:4812/apps/chat`.
- 2026-05-16: Created PRD 05 US-003..US-006 batch commit `feat: complete PRD05 CLI session command flows`; see git history for final hash. Next dependency-order target is `prd_06_ssh_recovery_hardening.json` US-001 TTY detection/non-TTY fallback hardening.
- 2026-05-16: Started PRD 06 SSH/recovery hardening batch (US-001..US-005). Plan: harden stdin/stdout TTY detection, make exit cleanup idempotent and shared across Ctrl+C/Slash exits/unmount, add redacted recovery error and empty-state messaging, verify default row-window/narrow-line bounds, document SSH recovery usage/checklist, then run Docker typecheck, focused tests, full tests, CLI non-TTY and pseudo-TTY smoke checks before marking PRD 06 passed.
- 2026-05-16: Implemented PRD 06 US-001..US-004: `pibo tui:sessions` now rejects non-interactive stdin or stdout before launching Ink, derives narrow-line bounds from terminal columns, shares an idempotent cleanup path for Ctrl+C/Slash exits/unmount, redacts source errors with recovery hints, handles empty session/room/profile states without broken pickers, and verifies default 20-row app viewport/narrow-line truncation in tests.
- 2026-05-16: Implemented PRD 06 US-005 docs: added SSH recovery usage, command/Slash Command notes, Web-only exclusions, non-TTY behavior, bounded transcript notes, and validation checklist to `06-ssh-recovery-hardening.md`; checked Phase 5/6 tasks and acceptance checklist in `tasks.md`.
- 2026-05-16: Validation for PRD 06 passed in Docker: `npm run typecheck`; `npm run build && node --test test/cli-ui-session-app.test.mjs`; full `npm test` (427 tests); pseudo-TTY startup/Ctrl+C smoke passed with `node dist/bin/pibo.js tui:sessions --demo --max-rows 5`; non-TTY CLI seam covered by focused tests. Marked `prd_06_ssh_recovery_hardening.json` US-001..US-005 `passes: true`.
- 2026-05-16: Reloaded Docker worker gateway after PRD 06 with container-only build/start commands. Health probes reset during rebuild/startup, then passed at `http://127.0.0.1:4812/apps/chat`.
- 2026-05-16: Created PRD 06 hardening/final validation batch commit `feat: harden PRD06 CLI session recovery`; all Ink CLI Session UI PRD JSON stories are now marked `passes: true` after validation.
