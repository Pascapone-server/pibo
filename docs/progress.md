...
- A `pibo.context-files` plugin now exposes managed global and agent-scoped markdown context files through `/api/context-files`, persists managed-file metadata, and emits product events when files are created, changed, removed, or updated on disk.
- The Chat Web App now includes a Context area at `/apps/chat/context` that reuses the managed context-file APIs inside the authenticated Chat shell. The older standalone `/apps/context-files` app still exists, but the integrated Chat area is the main operator workflow.
