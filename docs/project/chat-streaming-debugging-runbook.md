...
- **Markdown render**: renderer invocation count/plain/full split and total/max duration for assistant Markdown hosts when debug streaming is enabled; full-path timing includes synchronous ReactMarkdown parse and tree building, with GFM plugins only when the input uses GFM-specific syntax.
