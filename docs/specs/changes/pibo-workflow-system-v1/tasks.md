...
- [ ] 2.1 Add `text` and `json` port support.
- [ ] 2.2 Document the OpenAI Structured Outputs / tool-calling JSON Schema subset supported in V1.
...
- [ ] 3.1 Implement a one-node `agent` workflow path.
- [ ] 3.2 Ensure the node launches through normal Pibo session routing.
- [ ] 3.3 Persist workflow run id, workflow id, version, status, current node, input, and output.
...
- [ ] 4.1 Add edge validation for source and target node existence.
...
- [ ] 5.1 Add registered TypeScript adapter refs for `edgeAdapter` and visible `adapter` nodes.
