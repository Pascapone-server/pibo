...
- `gfm-task-markdown`: GFM task-list assistant deltas with simple inline links, including strong labels, that can exercise narrowly guarded task-list fast rendering while keeping small per-delta visible jumps.
- `gfm-full-markdown`: GFM task-list assistant deltas with richer Markdown inside link labels that must exercise the full `remark-gfm` parser while keeping small per-delta visible jumps.
