...
- Simple GFM task-list fast paths can safely include narrowly guarded balanced strong spans when the item text has no escapes, links, code, tables, strikethrough, or other nested syntax; keep `gfm-full-markdown` on inline-code task-list chunks so full `remark-gfm` parser coverage remains separate from `gfmFast` output.
- Simple GFM task-list fast paths can include narrowly guarded inline code spans as well as strong spans. Keep full `remark-gfm` fixture coverage on syntax the direct renderer rejects so `gfmFast` improvements do not hide parser regressions.
- Simple GFM task-list fast paths can include narrowly guarded inline links when the label and URL are plain and pass the same URL safety rules as the full renderer. Keep full `remark-gfm` fixture coverage on nested link-label Markdown so link fast paths do not hide parser regressions.
