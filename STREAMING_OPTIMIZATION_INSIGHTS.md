...
- The current `--simulate-reconnect` backend fixture still exposes a separate live-only gap: transient `live:<n>` frames are not resumable through the numeric `since` cursor, so forced EventSource closes can lose live-only text/reasoning even though the independent SSE probe receives all fixture events.
