...
- Simple GFM task-list fast paths can include narrowly guarded inline code spans as well as strong spans. Keep full `remark-gfm` fixture coverage on syntax the direct renderer rejects, such as task-list links, so `gfmFast` improvements do not hide parser regressions.
