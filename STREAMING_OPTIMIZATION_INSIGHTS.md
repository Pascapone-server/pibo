...
- Markdown render instrumentation should run only when streaming debug is enabled and split plain vs full Markdown paths. Production React builds do not report Profiler durations, so browser artifacts should use debug-only renderer self-duration unless a profiling build is explicitly used.
